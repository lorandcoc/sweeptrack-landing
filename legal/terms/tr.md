...
SweepTrack Pro, metal arama meraklıları için tasarlanmış GPS tabanlı bir takip ve oturum günlüğü uygulamasıdır. Uygulama şunları sağlar:

- Gerçek zamanlı GPS takibi ve yol kaydı
- Mesafe, hız, rakım ve süre metrikleriyle oturum günlüğü
- Fotoğraf, ses ve video ekleriyle buluntu/keşif kaydı
- Hava durumu tahmini ve arama koşulu puanlaması
- Gelgit tablosu tahminleri (NOAA aracılığıyla ABD kıyı istasyonları)
- Canlı grup konumlandırma (Radar): oluşturduğunuz veya katıldığınız bir arama grubuyla paylaşılan canlı harita
...
- Ayda 10'a kadar takip oturumu
- Sınırsız buluntu, her biri tek fotoğrafla (Pro; derinlik, değer, ağırlık, sinyal ve toprak notlarının yanı sıra ses, video ve çoklu fotoğraf yakalamanın kilidini açar)
- 1 dedektör ön ayar yuvası
- Bir Radar canlı grubuna ücretsiz katılım
- Temel takip ve haritalama özelliklerine erişim

### 5.2 Premium Abonelik

Google Play Store aracılığıyla yönetilen ve RevenueCat, Inc. tarafından işlenen premium abonelik, tam buluntu kaydının (derinlik, değer, ağırlık, sinyal ve toprak notlarının yanı sıra ses, video ve çoklu fotoğraf yakalama), sınırsız oturumların ve sınırsız dedektör ön ayarlarının kilidini açar; ayrıca tarihi harita kaplamaları, çevrimdışı haritalar, çevre koruyucu, bulut yedekleme, kendi Radar gruplarınızı barındırma ve ek araçlar dahil olmak üzere gelişmiş özellikleri sunar.
...
- **Kaydedilmiş hiçbir Uygulama verisi (oturumlar, buluntular, kasa) işlettiğimiz sunuculara iletilmez.** Tek istisna isteğe bağlı Canlı Grup özelliğidir (Radar): oluşturduğunuz veya katıldığınız bir grupta bulunduğunuz sürece, canlı konumunuzu ve paylaşılan harita öğelerinizi Google'ın Firebase Realtime Database'i aracılığıyla paylaşır — bkz. Gizlilik Politikamız, §3.5
- **Google Drive yedeklemesi** (isteğe bağlı, kullanıcı tarafından başlatılan), verilerinizin bir yedek kopyasını kişisel Google Drive hesabınızda "SweepTrack Backups" klasörüne kaydeder. Parola ile yedekleme şifrelemesini etkinleştirirseniz, yedek yüklemeden önce cihazınızda şifrelenir
...
- **Open-Meteo** — Hava durumu verileri ve yer araması (yaklaşık koordinatlar gönderilir, kişisel bilgi yok)
- **NOAA** — Gelgit tahminleri, ABD kıyı istasyonları (istasyon kimliği gönderilir, kişisel bilgi yok)
- **Firebase Realtime Database** (Google) — Canlı Grup arka ucu, yalnızca bir Radar grubu oluşturduğunuzda veya katıldığınızda (bkz. §6.2 ve Gizlilik Politikası §3.5)
- **RevenueCat** — Abonelik doğrulaması (anonim uygulama kullanıcı kimliği, kişisel bilgi yok)

Ters geocoding (koordinatları bir yer adına dönüştürme) cihazda Android'in platform geocoder'ı aracılığıyla çalışır ve bir üçüncü taraf ağ çağrısı değildir.
...
Uygulamadaki buluntu kategorileri (Hazine, Altın, Madeni Para vb.) yalnızca kişisel organizasyon içindir. Uygulama yasal sınıflandırma, arkeolojik önem, değer veya raporlama yükümlülüklerini **belirlemez**.

### 8.5 Yasal Tavsiye Yok

Uygulamada hiçbir şey — İzin Kasası, hava puanı, arama tahmini, tarihi harita kaplamaları, buluntu kategorizasyonu veya herhangi bir metin dahil — herhangi bir türde yasal, finansal, arkeolojik veya profesyonel tavsiye oluşturmaz.
...
Uygulama Google Maps, OpenStreetMap, Esri, OpenTopoMap, USGS, Open-Meteo ve NOAA'dan veri kullanır. Harita verileri yasal mülkiyet sınırlarını veya erişim haklarını belirlemek için **asla** kullanılmamalıdır.
...
**Gelgit tahminleri** fırtına dalgalanmasını, rüzgarı veya yerel anormallikleri hesaba katmayabilir. **Çevrimdışı haritalar** güncelliğini yitirebilir. **Dedektör ön ayarları** üreticiler tarafından onaylanmamış organizasyon araçlarıdır.
...
Uygulama açık kaynak bileşenler kullanır. Önemli atıflar: Google Maps SDK (Copyright Google LLC), OpenStreetMap (ODbL), OpenTopoMap, Open-Meteo (CC BY 4.0), NOAA (kamu malı), USGS (kamu malı), Jetpack Compose/Room/Hilt (Apache 2.0), Firebase SDK (Google), RevenueCat SDK (MIT).

...
Az Alkalmazás **adatvédelem-központú architektúrával** készült: az Ön elmentett keresési adatai — munkamenetek, találatok, kasszabejegyzések — helyben, az Ön eszközén tárolódnak, ezeket nem értékesítjük, és nem használjuk profilalkotásra. Bizonyos adatok kizárólag a 3.2., 3.3. és 3.5. szakaszban leírt, az Ön által kezdeményezett funkciók céljából hagyják el az eszközét: valós idejű API-funkciók, opcionális biztonsági mentés, opcionális diagnosztika, opcionális visszajelzés, valamint — ha Ön úgy dönt, hogy csoportot hoz létre vagy csatlakozik hozzá — az opcionális Live Group helymegosztás, amely a Radar funkció teljes célja. A Weboldal külön működik, és saját adatkezelési gyakorlattal rendelkezik, amelyet a 4. szakasz ismertet.
...
- Találat típusa (Kincs, Arany, Érme, Relikvia, Ékszer, Hulladék, Besorolatlan/Gyors mentés)
- Találat helye (GPS-koordináták a rögzítés időpontjában)
- Opcionális metaadatok: név, jegyzetek, mélység, becsült érték, súly
- Médiamellékletek: fényképek, hangfelvételek és videófelvételek
...
**Útpontok:** A felhasználó által elhelyezett térképjelölők — koordináták, név, kategória és opcionális jegyzetek.
...
- Koordináták helynevekké való feloldása az Android platform geokódolóján keresztül (a legtöbb eszközön a Google Play Services biztosítja) fordított geokódolás céljából — a munkamenetek és találatok helynévvel való ellátására használjuk
- Árapály állomás azonosítók a NOAA-nak árapály-előrejelzésekhez (egyesült államokbeli tengerparti állomások)
- Helyszínkeresési lekérdezések az Open-Meteo Geocodinghoz
- Térképcsempe-kérések (amelyek felfedik az Ön által megtekintett térképterület hozzávetőleges helyét) az aktív térképtípus csempeszolgáltatójához küldve: Esri/ArcGIS (műholdképek és USGS topográfiai térképek), OpenStreetMap és OpenTopoMap (offline/letölthető térképcsomagok), valamint — a román történelmi „Régi térkép" rétegnél — egy általunk a Cloudflare-en üzemeltetett gyorsítótárazó proxy, amely a geo-spatial.org-ról (eHarta) tölti le a csempéket. Az alapértelmezett Google térképalapot a 3.3. szakaszban szereplő Google Maps SDK fedi le
...
**Alkalmazáson belüli visszajelzés (opcionális):** Ha az Alkalmazás visszajelzési űrlapján keresztül küld visszajelzést, az Alkalmazás továbbítja az Ön által megadott információkat — az üzenetét, a kiválasztott kategóriát, valamint azt, hogy hibajelentésről vagy funkcióötletről van-e szó — az eszköze modelljével, az Android verziójával, az alkalmazás verziójával és a nyelvi beállításával együtt, valamint **kizárólag akkor, ha úgy dönt, hogy megadja, az e-mail-címét** egy általunk üzemeltetett Supabase Edge Functionnek, hogy elolvashassuk és válaszolhassunk. Semmi sem kerül elküldésre, hacsak Ön be nem küldi az űrlapot. A Supabase, Inc. adatfeldolgozónkként jár el (EU-régió). Vonatkozik rá a [Supabase Adatvédelmi szabályzata](https://supabase.com/privacy).

### 3.4 Adatok, amelyeket az Alkalmazás NEM gyűjt

- **Az Alkalmazás** nem gyűjti az Ön nevét, telefonszámát vagy bármely más személyes azonosítóját — kivéve egy e-mail-címet, amelyet Ön opcionálisan ad meg az alkalmazáson belüli visszajelzés beküldésekor (lásd a 3.3. szakaszt)
...
- **Az Alkalmazás** nem értékesíti, nem béreli ki és nem cseréli ki az adatait, és csak ott osztja meg, ahol Ön kezdeményezi: a 3.2. szakaszban szereplő valós idejű API-hívások, az opcionális Google Drive biztonsági mentés, az opt-in Firebase diagnosztika, az opcionális Live Group helymegosztás (3.5. szakasz), valamint az Ön által beküldeni választott visszajelzés (3.3. szakasz)
- **Az Alkalmazás** nem üzemeltet olyan szervereket, amelyek tárolják az Ön elmentett keresési adatait (munkamenetek, találatok, kasszabejegyzések) — egyetlen kivétellel, amelyet Ön kontrollál: az opcionális Live Group funkció (3.5. szakasz), amely az Ön élő helyzetét megosztja egy Ön által létrehozott vagy megnyitott csoporttal, a Google Firebase Realtime Database-én keresztül, amíg Ön abban a csoportban van. A Live Group funkción kívül az egyetlen tartalom, amely egy általunk üzemeltetett szerverre eljut, az Ön által beküldeni választott visszajelzés (3.3. szakasz)

A Weboldal adatkezelési gyakorlatát külön a 4. szakasz ismerteti.

### 3.5 Live Group helymegosztás (opcionális — a Radar funkció)

A SweepTrack Pro tartalmazza a **Radart**, egy élő csoportos helymeghatározási funkciót, amely a kezdőtérkép RADAR gombjáról érhető el. Ez kikapcsolva marad, hacsak Ön nem **hoz létre csoportot vagy nem csatlakozik hozzá**. Egy csoport létrehozása (házigazdaként) Pro funkció; egy csoporthoz való csatlakozás ingyenes. Amíg Ön egy csoportban van, a következők kerülnek továbbításra a Google **Firebase Realtime Database**-ébe, és válnak valós időben láthatóvá ugyanazon csoport többi tagja számára:

- **Az Ön megjelenített neve** — egy Ön által választott név; nem kell a valódi nevének lennie.
- **Az Ön pontos helye** — GPS-szélesség és -hosszúság pontossággal és iránnyal, amely frissül, amíg Ön a csoportban van (kb. 30 másodpercenként az ingyenes szinten, gyakrabban előfizetéssel, és néhány másodpercenként egy SOS során).
- **Az Ön állapota** — online, szüneteltetett vagy mozdulatlan, valamint hogy elindított-e **SOS**-t (beleértve az adott pillanatbeli helyét is).
- **Az Ön eszközének akkumulátorszintje, térképjelölőjének színe és ikonja**, valamint bármely Ön által létrehozott **megosztott útpont, ping vagy bázis-/gyülekezőpont**, mindegyik az Ön megjelenített nevével és koordinátáival ellátva.

**Ki láthatja:** kizárólag a csoport tagjai. Bárki, aki rendelkezik a csoport csatlakozási kódjával vagy QR-kódjával, csatlakozhat és láthatja az élő térképet, ezért csak olyan emberekkel ossza meg a kódot, akikkel meg kívánja osztani a helyzetét. A hozzáférést szerveroldali biztonsági szabályok érvényesítik; egy személynek csatlakoznia kell a csoporthoz (ezáltal maga is láthatóvá válva), mielőtt olvashatná a pozíciókat.

**Hitelesítés:** a csoport háttérrendszere a **Firebase Anonymous Authentication**-t használja — egy véletlenszerű azonosítót az Ön alkalmazástelepítéséhez, amely nem kapcsolódik az Ön nevéhez, e-mail-címéhez, telefonszámához vagy Google-fiókjához.

**Meddig őrizzük:** a csoport helymeghatározási adatai átmenetiek. Egy csoport **lejár** (jellemzően kb. 12 óra után; a próbacsoportok hamarabb), ezt követően nem fogad több pozíciót, és nem lehet többé csatlakozni hozzá. Egy csoportból való kilépés azonnal törli az Ön tagsági bejegyzését, és a csoport törlődik, amikor az utolsó tagja kilép. Ha egy csoportot úgy hagynak magára, hogy senki sem lép ki, a lejárat előtt megosztott utolsó pozíciók a lejárt csoportban maradnak tárolva, amíg az törlődik. Nem őrzünk hosszú távú előzményt az Ön mozgásairól a szervereinken, és Ön bármikor kérheti a törlést (18. szakasz).

Az önálló, ingyenes kísérőalkalmazásnak saját, részletesebb tájékoztatója van a [SweepTrack Radar Adatvédelmi szabályzatában](https://sweeptrack.pro/privacy-radar).
...
- **API-hívások (időjárás, geokódolás, térképcsempék):** Jogos érdek (6. cikk (1) bekezdés f) pont) — szükséges az alapfunkciókhoz
...
- **Live Group helymegosztás (opcionális):** Hozzájárulás (6. cikk (1) bekezdés a) pont) — Ön aktívan létrehoz egy csoportot vagy csatlakozik hozzá, a megosztás aktív állapota alatt előtérben megjelenő értesítéssel; a hozzájárulás a csoportból való kilépéssel vagy a funkció bezárásával vonható vissza
- **Alkalmazáson belüli visszajelzés (opcionális):** Hozzájárulás (6. cikk (1) bekezdés a) pont) — csak akkor kerül elküldésre, ha Ön beküldi a visszajelzési űrlapot
...
Az Alkalmazás szinte minden adatfeldolgozása **az Ön eszközén helyben** történik. Az Alkalmazás adatait az Alkalmazás funkcióinak biztosításához használjuk: térkép megjelenítése, munkamenet-nyomkövetés, találatok rögzítése, engedélykezelés, időjárási/árapály adatok, exportok, biztonsági mentés és előfizetés-ellenőrzés. Az adatok kizárólag a 3.2. és 3.3. szakaszban leírt, az Ön által kezdeményezett funkciók céljából hagyják el az eszközét (valós idejű API-k, opcionális biztonsági mentés, opcionális diagnosztika, opcionális visszajelzés).
...
- A támogatott eszközökön az érzékeny beállítások és az Engedélykassza adatai az Android EncryptedSharedPreferences-ét használják (AES-256), amelyet az Android Keystore támogat. Ha a Keystore vagy az EncryptedSharedPreferences nem érhető el egy eszközön, az Alkalmazás az alkalmazás-privát (elszigetelt) tárhelyre vált vissza, és tartós, alkalmazáson belüli figyelmeztetést jelenít meg arról, hogy ezek az adatok az adott eszközön nincsenek külön titkosítva
...
- A Google Drive biztonsági mentések HTTPS/TLS protokollon keresztül kerülnek továbbításra, és kizárólag az Ön saját fiókjában tárolódnak. Egy biztonsági mentést opcionálisan jelszóval védhet, amely feltöltés előtt az Ön eszközén titkosítja az archívumot (AES-256-GCM), így az a jelszó nélkül nem olvasható
- Az Ön által beküldött opcionális, alkalmazáson belüli visszajelzés HTTPS/TLS protokollon keresztül kerül továbbításra egy Supabase Edge Functionhez (EU-régió)

Az Alkalmazás esetében nem üzemeltetünk olyan szervereket, amelyek tárolják az Ön elmentett keresési adatait. Az egyetlen kivétel az opcionális Live Group funkció (3.5. szakasz), amely átmeneti, automatikusan lejáró csoport helymeghatározási adatokat tárol a Google Firebase-ében, amíg Ön egy csoportban van.
...
Nem értékesítjük, nem béreljük ki és nem cseréljük ki az Ön személyes adatait. Választhatja az Alkalmazás adatainak megosztását exportokon keresztül (GPX, KML, CSV, JSON), munkamenet-megosztási kártyákon, a Google Drive biztonsági mentésén, vagy egy Radar-csoportban történő élő helymegosztáson keresztül (3.5. szakasz) — mindegyiket a felhasználó kezdeményezi. Ha alkalmazáson belüli visszajelzést küld be, azt a nevünkben a Supabase dolgozza fel (lásd a 3.3. szakaszt). A Weboldal várólista adatait csak a megnevezett adatfeldolgozóink (Supabase, Resend) dolgozzák fel a 4. szakaszban leírt célokra.
...
- **INTERNET** — időjárás, geokódolás, árapályok, térképek, előfizetések, opcionális visszajelzés
...
**Az Alkalmazás adatai:** Mivel az összes keresési adatot helyben tároljuk az eszközén, az ellenőrzésünk alatt nem történik nemzetközi adattovábbítás. A harmadik felekhez intézett API-hívások (3.2. szakasz), az opcionális Google Drive biztonsági mentés, az opcionális Firebase diagnosztika, az opcionális Live Group helymegosztás (3.5. szakasz, a Google Firebase-én keresztül) és az opcionális visszajelzés feldolgozhatók azokban a joghatóságokban, ahol ezek a szolgáltatások működnek.
...
**Alkalmazás:** Az Ön elmentett keresési adatai az eszközén tárolódnak, nem az általunk üzemeltetett szervereken, így azokat nem teszi ki egy, az infrastruktúránkat érintő incidens. Az egyetlen kivétel az opcionális Live Group funkció (3.5. szakasz): amíg Ön egy csoportban van, az élő helyzete átmenetileg a Google Firebase-ében tárolódik, amelyet szerveroldali szabályok az adott csoport tagjaira korlátoznak. Ha tudomásunkra jut egy sebezhetőség az Alkalmazásban, frissítést adunk ki, és értesítjük a felhasználókat az Alkalmazáson vagy a weboldalon keresztül. Az Ön által beküldött opcionális visszajelzést adatfeldolgozónk, a Supabase tárolja, saját incidens-értesítési protokolljai szerint.
...
- [Google (Maps, Drive, Sign-In, platform geokódoló)](https://policies.google.com/privacy)
...
- [Cloudflare](https://www.cloudflare.com/privacypolicy/) (történelmi térkép csempeproxy)
- [NOAA](https://www.noaa.gov/privacy-policy)
- [Vercel](https://vercel.com/legal/privacy-policy) (Weboldal-tárhelyszolgáltatás + elemzés)
- [Supabase](https://supabase.com/privacy) (Weboldal-adatbázis + alkalmazáson belüli visszajelzés)

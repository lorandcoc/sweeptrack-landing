...
const socials = [
  { name: "Facebook", icon: <FacebookIcon />, link: "https://www.facebook.com/sweeptrackpro/" },
  { name: "YouTube", icon: <YoutubeIcon />, link: "https://www.youtube.com/@SweepTrackPro" },
  { name: "Instagram", icon: <InstagramIcon />, link: "https://www.instagram.com/sweeptrackpro/" },
  { name: "TikTok", icon: <TiktokIcon />, link: "https://www.tiktok.com/@sweeptrackpro" },
] as const;
...
export default function CommunityLinks() {
  ...
  return (
    <section id="community" className="py-20 md:py-28 relative">
      ...
      <div ref={ref} className={`relative z-10 max-w-3xl mx-auto px-6 reveal ${visible ? "visible" : ""}`}>
        {/* Heading + description */}
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5">
            {socials.map((s) => (
              <a
                ...
                className="ctx-social inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/[0.08] bg-white/[0.02] text-muted text-sm transition-colors"
              >
                {s.icon}
                <span>{s.name}</span>
              </a>

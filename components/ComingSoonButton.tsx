...
export default function ComingSoonButton({
  ...
  if (status === "ok") {
    return (
      <div className={`inline-flex items-center justify-center gap-2 ${padClass} rounded-lg bg-accent/15 border border-accent/30 text-accent font-semibold ${className}`}>
      ...
      <div className={`flex flex-col gap-2 w-full sm:w-auto ${className}`}>
        <form onSubmit={handleSubmit} className="inline-flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          <input
            ...
            className="px-4 py-3 rounded-lg bg-surface border border-white/10 text-sm text-foreground placeholder:text-muted/60 focus:outline-none focus:border-accent/40 transition-colors w-full sm:w-[220px] disabled:opacity-50"
          />
          ...
          <button
            type="submit"
            disabled={status === "sending"}
            className="px-5 py-3 rounded-lg bg-accent text-[#050510] font-semibold text-sm hover:bg-accent-dim transition-colors shrink-0 disabled:opacity-50"
          >
  ...
  if (status === "error") {
    return (
      <button
        ...
        className={`group inline-flex items-center justify-center gap-3 ${padClass} rounded-lg bg-red-500/15 border border-red-500/30 text-red-400 font-semibold ${className}`}
      >
        {t("comingsoon.error")}
      </button>
    );
  ...
  return (
    <button
      ...
      className={`group inline-flex items-center justify-center gap-3 ${padClass} rounded-lg bg-accent text-[#050510] font-semibold hover:bg-accent-dim transition-colors ${className}`}
    >
